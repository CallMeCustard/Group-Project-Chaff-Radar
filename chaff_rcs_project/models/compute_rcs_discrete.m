function sigma_total = compute_rcs_discrete(N, lambda, L_mean, L_std, spread_sigma, radar_vec, pol_vec)
...
lambda = 0.032;               % ~9.4 GHz
N = 1000;                     % Number of chaff dipoles
L_mean = lambda / 2;          % Half-wave dipole
L_std = L_mean * 0.2;         % 20% variation
spread_sigma = 2;             % Spread in meters
radar_vec = [1; 0; 0];        % Radar incoming along x-axis
pol_vec = [0; 1; 0];          % Polarised along y-axis

sigma = compute_rcs_discrete(N, lambda, L_mean, L_std, spread_sigma, radar_vec, pol_vec);
fprintf('Total chaff cloud RCS: %.4f m^2\n', sigma);
